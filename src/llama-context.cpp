...
void llama_context::set_kq_key_veto_from(llama_pos p) {
    LLAMA_LOG_DEBUG("%s: p = %d\n", __func__, p);

    cparams.kq_key_veto_from = p;
}
...
void llama_set_kq_key_veto_from(llama_context * ctx, llama_pos p) {
    ctx->set_kq_key_veto_from(p);
}
