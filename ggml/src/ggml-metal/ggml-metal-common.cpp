...
// must stay in sync with the kernel_fwht_<type>_<N> templates in misc.metal (prism: the
// threadgroup kernel covers the wide folded-Hadamard widths up to 8192)
static bool ggml_metal_fwht_supported_size_common(int64_t n) {
...
bool ggml_metal_op_mul_mat_use_fwht(const struct ggml_tensor * op) {
    return ggml_get_op_params_i32(op, 1) == GGML_HINT_SRC0_IS_HADAMARD &&
           ...
           ggml_metal_fwht_supported_size_common(op->src[1]->ne[0]);
}
...
bool ggml_metal_op_mul_mat_use_mm(const struct ggml_tensor * op, bool has_simdgroup_mm) {
    ...
    // prism: Q1_0 stays on the (multi-column) mul_mv kernels up to GGML_METAL_Q1_0_MV_MAX rows
    // (default 16): the generic mat-mat path is slower per weight pass for small batches
    static const int q1_0_mv_max = getenv("GGML_METAL_Q1_0_MV_MAX") ? atoi(getenv("GGML_METAL_Q1_0_MV_MAX")) : 16;
    const int64_t ne11_mm_min = op->src[0]->type == GGML_TYPE_Q1_0 ? std::max(8, q1_0_mv_max) : 8;
