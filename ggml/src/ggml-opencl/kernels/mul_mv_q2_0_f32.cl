...
// GluRun (patch 0006): Q2_0 (ternary, QK2_0 = 64) mat-vec, the structure of
// mul_mv_q1_0_f32.cl with the dequant swapped. Element j of a block lives in
// byte j/4 at bit offset 2*(j%4); the 2-bit code q maps to (q - 1) * d, i.e.
// {-1, 0, +1, +2} * d, exactly as ggml-cpu's dequantize_row_q2_0.
#define QK2_0 64
typedef struct {
    half d;
    uchar qs[QK2_0/4];
...
// sum_j y_j * (q_j - 1) = sum_j y_j * q_j - sumy
...
kernel void kernel_mul_mv_q2_0_f32(
    ...
    // 4 threads per block (16 quants each), N_SIMDWIDTH/4 blocks per step
