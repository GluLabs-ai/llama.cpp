...
static bool ggml_backend_vk_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_GATED_DELTA_NET:
            {
                // prism: rows-indexed state read (src[6], ggml_gated_delta_net_rows) is not
                // implemented by gated_delta_net.comp; decline it so the CPU computes it
                if (op->src[6] != nullptr) {
                    return false;
                }
                // prism: raw gates (ggml_gated_delta_net_set_raw_gates): beta and g arrive
